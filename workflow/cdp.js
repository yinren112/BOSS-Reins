...
async function httpJson(url) {
  ...
  const body = await request('GET', `${parsed.pathname}${parsed.search}`, Number(parsed.port || DEFAULT_PORT));
...
class CDP {
  ...
  async connectPage(match = 'zhipin.com') {
    ...
    const page = tabs.find(tab => tab.type === 'page' && test(tab));
    if (!page) throw new Error(`9222 未找到匹配页面（${typeof match === 'function' ? '自定义条件' : match}）`);
  ...
  async navigate(url) {
    try {
      ...
      // SPA 离开当前页面时，旧页面的卸载监听偶尔会抛业务 JS 异常；导航本身已经成功。
      // 回读目标 URL 决定是否继续，避免把站点卸载错误误报成浏览器“未就绪”。
      await sleep(200);
...
async function openTab(url, port = DEFAULT_PORT) {
  const raw = await request('PUT', `/json/new?${encodeURIComponent(url)}`, port);
