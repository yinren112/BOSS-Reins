...
const ARCHIVE = path.join(ROOT, 'archive', '2026-07-17_legacy');
...
const { hydrateLegacyStructured, jdHashOf, parseJobBody } = require('./jd-domain');
...
// 一次性离线迁移：福利归一化改了 hash 算法，台账里旧算法存的 hash 必须按同一算法重算，
// 否则每条待发送岗位都会在发送门禁上误报「JD 已变化」。不联网、不占额度。
function rehashJd() {
  const ledger = loadLedger();
  ...
  for (const job of ledger.jobs) {
    ...
    if (!structured || !structured.description) { result.skippedNoStructured++; continue; }
    const next = jdHashOf(structured);
    const prev = job.jd.hash;
    if (next === prev) continue;
    // 开场白原本绑在旧 hash 上就跟着换绑；绑的是别的 hash 说明 JD 确实变过，保持失配让门禁拦住。
    if (job.opener?.jdHash && job.opener.jdHash === prev) {
...
function validate({ quiet = false } = {}) {
  ...
  for (const job of ledger.jobs) {
    ...
    if (/^delivered/.test(job.outreach?.status || '')) {
      ...
      const validRowEvidence = v.inputEmpty && v.exactMessage && v.sameRowDelivered && v.companyVisible;
      const validIdentityEvidence = v.inputEmpty && v.identityMatched && v.delivered && v.companyVisible;
      if (!(validLegacy || validRowEvidence || validIdentityEvidence)) errors.push(`送达核验不完整: ${job.jobId}`);
    }
  }
...
module.exports = { migrateJd, rehashJd, importLegacy, validate };
